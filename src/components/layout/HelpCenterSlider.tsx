...
const HelpCenterSlider: React.FC<HelpCenterSliderProps> = ({
  open,
  onClose,
}) => {
  ...
  useEffect(() => {
    const handler = () => {
      try {
        ...
        const arr = raw ? JSON.parse(raw) : [];
        setCompletedPaths(
          Array.isArray(arr) ? new Set<string>(arr) : new Set<string>()
        );
      } catch {}
    };
  ...
  const completedCount = setupWizardSteps.filter(
    (item) => item.completed
  ).length;
  ...
  const otherMenuItems = [
    ...
    {
      title: "Improve your Ajaxter skills",

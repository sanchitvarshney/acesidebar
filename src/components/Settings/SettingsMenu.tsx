...
const SettingsMenu = ({
  ...
  return (
    <aside className="w-80 min-w-[320px] border-r border-gray-200 bg-white flex flex-col h-[calc(100vh-90px)] p-4 overflow-y-auto shadow-sm">
      {menuSections.length > 0 ? (
        menuSections.map((section: any) => (
          <div
            key={section.id}
            className={`flex items-start gap-4 mb-3 cursor-pointer p-4 rounded-lg transition-all duration-300 group relative
              ${
                activeId === section.id

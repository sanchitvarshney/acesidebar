...
const DepartmentsManagement = () => {
  ...
  const [activeFilters, setActiveFilters] = useState<Array<{ id: string, field: string, operator: string, value: string, label: string }>>([]);
  const [filterMenuAnchor, setFilterMenuAnchor] = useState<null | HTMLElement>(null);
  const [filterDialogOpen, setFilterDialogOpen] = useState(false);
  const [selectedFilterField, setSelectedFilterField] = useState<{ field: string, label: string, type?: string, options?: string[], operators?: string[], defaultOperator?: string } | null>(null);
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>([
    'departmentName', 'departmentType', 'manager', 'agentCount', 'isActive'
  ]);
  const [getDepartmentList, { data: departmentList, isLoading: isGetDepartmentListLoading }] = useLazyGetDepartmentListQuery();
  ...
  const fieldOptions = [
    ...
    },
    {
      ...
      icon: <BusinessIcon />,
      type: 'multiCheckbox',
      options: ['Support', 'Sales', 'Technical', 'Administrative', 'Finance', 'HR', 'Operations'],
      operator: 'equals'
    },
  ];
  ...
  const applyFilter = () => {
    ...
    if (selectedFilterField && isValidFilter) {
      const existingFilterIndex = activeFilters.findIndex(filter => filter.field === selectedFilterField.field);
  ...
  const filteredColumns = departmentColumns.filter(column =>
    ...
    <Box sx={{
      ...
      <Box sx={{ p: 3, display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        >
          ...
          <Box
            sx={{
              ...
              '&:hover': activeFilters.length < 5 ? {
                backgroundColor: '#f8f9fa',
              } : {}
            }}
            onClick={activeFilters.length < 5 ? (event) => setFilterMenuAnchor(event.currentTarget) : undefined}
          >
            ...
            {activeFilters.length > 0 && (
              <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap sx={{ alignItems: 'center' }}>
                {activeFilters.map((filter, index) => (
                  <Chip
                    key={filter.id}
                    ref={index === activeFilters.length - 1 ? setLastChipRef : null}
        ...
        <Card sx={{ flex: 1, boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
          <TableContainer sx={{ height: "calc(100vh - 300px)", minHeight: "400px", position: 'relative' }}>
                    ...
                    >
                      ...
                      <TableCell>
                        <Chip
                          ...
                          size="small"
                          sx={{
                            bgcolor: row.isActive === false ? '#ffebee' : '#e8f5e8',
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          <Card sx={{ boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
            <CardContent>
              ...
              <Typography variant="body2" sx={{ lineHeight: 1.6, color: '#65676b' }}>
                Departments help you organize agents based on functions such as{" "}
                <Box component="span" sx={{ fontWeight: 600, color: '#1877f2' }}>Support</Box>,{" "}
                <Box component="span" sx={{ fontWeight: 600, color: '#1877f2' }}>Sales</Box>, or{" "}
                <Box component="span" sx={{ fontWeight: 600, color: '#1877f2' }}>Technical</Box>.
                This makes it easier to assign tickets, manage workflows, and generate
                department-level reports. An agent can be part of one or more departments
          ...
          <Card sx={{ boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
            <CardContent>
              ...
              <Typography variant="body2" sx={{ lineHeight: 1.6, color: '#65676b' }}>
                ...
                for <Box component="span" sx={{ fontWeight: 600, color: '#1877f2' }}>Support</Box> and{" "}
                <Box component="span" sx={{ fontWeight: 600, color: '#1877f2' }}>Sales</Box>,
          ...
          <Card sx={{ boxShadow: '0 2px 8px rgba(0,0,0,0.1)' }}>
            <CardContent>
              ...
              <Typography variant="body2" sx={{ lineHeight: 1.6, color: '#65676b', mb: 2 }}>
              ...
              <Typography variant="body2" sx={{ lineHeight: 1.6, color: '#65676b', mb: 2 }}>
                Set up rules to automatically route tickets to the right department.
              </Typography>
              ...
              <Box component="ul" sx={{ pl: 2, m: 0, '& li': { mb: 1 } }}>
                <Typography component="li" variant="body2" sx={{ lineHeight: 1.6, color: '#65676b' }}>
                  <Box component="span" sx={{ fontWeight: 600, color: '#1877f2' }}>Round robin:</Box> Assign tickets to agents in a circular fashion.
                </Typography>
                <Typography component="li" variant="body2" sx={{ lineHeight: 1.6, color: '#65676b' }}>
                  <Box component="span" sx={{ fontWeight: 600, color: '#1877f2' }}>Load balanced:</Box> Limit the number of tickets per agent to balance workload.
                </Typography>
                <Typography component="li" variant="body2" sx={{ lineHeight: 1.6, color: '#65676b' }}>
                  <Box component="span" sx={{ fontWeight: 600, color: '#1877f2' }}>Skill based:</Box> Assign tickets based on agent expertise within the department.
                </Typography>
              </Box>
            </CardContent>
          </Card>
      ...
      {/* Filter Options Menu */}
      <Menu
        anchorEl={activeFilters.length > 0 && lastChipRef ? lastChipRef : filterMenuAnchor}
      ...
      >
        {fieldOptions.map((option) => (
          <MenuItem
            ...
            disabled={activeFilters.some(filter => filter.field === option.value)}
            sx={{
              py: 1,
              px: 2,
              fontSize: '0.875rem',
      ...
      >
        <Paper elevation={0}>
          ...
          <Box sx={{ p: 2 }}>
            ...
            >
              {selectedFilterField?.label} - {operatorOptions.find(op => op.value === selectedOperator)?.label}
            </Typography>
            ...
            {selectedFilterField?.type === 'multiCheckbox' ? (
              ...
              <TextField
                ...
                variant="standard"
                placeholder={
                  selectedFilterField?.field === 'departmentName' ? 'Enter department name...' :
                    selectedFilterField?.field === 'manager' ? 'Enter manager name...' :
                      selectedFilterField?.field === 'agentCount' ? 'Enter number...' :
                        'Enter value...'
                }
                value={filterValue}

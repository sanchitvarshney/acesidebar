...
import { Box, Typography, TextField, IconButton, Accordion, AccordionSummary, AccordionDetails, CircularProgress } from '@mui/material';
import { Person, OpenInNew, ExpandLess, ExpandMore, Email, Phone, Business, EditNote, ModeEdit, Save, Close } from '@mui/icons-material';
...
const CustomerInfoSection: React.FC<CustomerInfoSectionProps> = ({ isExpanded, onToggle, ticketData }) => {
  ...
  useEffect(() => {
    if (ticketData) {
      ...
      setInternalNote(ticketData.internalNotes ?? ticketData.internal_notes ?? '');
    }
  }, [ticketData]);
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
          <Person sx={{ color: '#666', marginRight: '8px', fontSize: '20px' }} />
          ...
          {isLoading ? (
            ...
            <IconButton size="small" onClick={toggleEdit} sx={{ padding: '4px', marginRight: '4px' }}>
              {isEditing ? <Save sx={{ fontSize: '16px', color: '#666' }} /> : <ModeEdit sx={{ fontSize: '16px', color: '#666' }} />}
            </IconButton>
          )}
          {isEditing && (
            <IconButton size="small" onClick={() => setIsEditing(false)} sx={{ padding: '4px' }}>
      ...
      <AccordionDetails sx={{ padding: '16px' }}>
        ...
        <Box sx={{ marginBottom: '12px' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
            <Email sx={{ fontSize: '16px', color: '#666', marginRight: '8px' }} />
        ...
        <Box sx={{ marginBottom: '12px' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
            <Phone sx={{ fontSize: '16px', color: '#666', marginRight: '8px' }} />
            <Typography variant="caption" color="text.secondary">
              Phone
            </Typography>
          </Box>
        ...
        <Box sx={{ marginBottom: '12px' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
            <Business sx={{ fontSize: '16px', color: '#666', marginRight: '8px' }} />
            <Typography variant="caption" color="text.secondary">
              Extension
            </Typography>
          </Box>
        ...
        <Box sx={{ marginBottom: '12px' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', marginBottom: '4px' }}>
            <EditNote sx={{ fontSize: '16px', color: '#666', marginRight: '8px' }} />
            <Typography variant="caption" color="text.secondary">
              Internal Notes
            </Typography>
          </Box>

...
import { Task } from "./drawer.taskType";
import { fieldOptions, getConditionOptions, priorityOptions } from "../task/data/taskData";
import { canEditComment, getStatusColor, getStatusIcon, validateComment, validateSearchCondition } from "../task/utils/taskUtils";
import DrawerTaskHeader from "./DrawerTaskHeader";
import DrawerTaskList from "./DrawerTaskList";
import TaskDetailsSkeleton from "../task/components/TaskDetailsSkeleton";
import TaskDetails from "../task/components/TaskDetails";
import CommentForm from "../task/components/CommentForm";
...
const DrawerTask: React.FC<TaskPropsType> = ({ isAddTask, ticketId }) => {
  ...
  const addCondition = () => {
    ...
    const availableFields = fieldOptions?.filter(
      (option:any) => !usedFields.includes(option.value)
    );
  ...
  const getAvailableFields = (currentConditionId: string) => {
    const usedFields = taskSearchConditions
      ...
      .map((c) => c.field);
    return fieldOptions?.filter((option:any) => !usedFields.includes(option.value));
  };
          ...
          <div className="flex flex-1 h-0 min-h-0">
            {!isAddTask && <LeftMenu />}
                ...
                <DrawerTaskList
                  ...
                  onAdvancedSearchOpen={(e:any) => handleTaskAdvancedSearchOpen(e)}
                        ...
                        {rightActiveTab === 0 && (
                          ...
                          >
                            <TaskDetails
                              ...
                              getStatusIcon={getStatusIcon}
                              onStatusChange={(taskId:any, newStatus:any) =>
                            ...
                            <Card>
                              <CardContent className="p-4">
                                ...
                                <CommentForm
                                  ...
                                  error={commentError}
                                  onCommentChange={(text:any) => {
                                    setNewComment(text);
                                    const error = validateComment(text);
                                    setCommentError(error);
                                  }}
                                  onInternalChange={(isPrivate:any) => {
                          ...
                          >
                            ...
                            >
                              ...
                              {!getAvailableFields(condition.id).some(
                                (opt:any) => opt.value === condition.field
                              ) &&
                                condition.field && (

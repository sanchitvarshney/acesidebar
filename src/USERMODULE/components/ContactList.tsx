...
const ContactList = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>([
    'contact', 'emailAddress', 'company', 'phoneNumber', 'roleType', 'facebook', 'twitter'
  ]);
      ...
      return activeFilters.every(filter => {
        ...
        switch (filter.operator) {
          ...
          case 'matches':
            ...
            const pattern = filterValueLower.replace(/\*/g, '.*').replace(/\?/g, '.');
  ...
  // Apply filters to data
  let filterData = agentList || [];

  // Apply custom filters only
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          <DataGrid
            ...
            loading={agentListData || !agentList} // Show loading state when data is being fetched
            initialState={{
          ...
          <Box sx={{ 
            ...
            <Box sx={{ flex: 1, p: 3, overflow: 'auto' }}>
                  ...
                  .map((column) => (
                    ...
                    >
                      ...
                      <IconButton
                        ...
                        sx={{ 
                          color: visibleColumns.includes(column.field) ? '#d32f2f' : '#999',
              ...
              <Box>
                ...
                {columns
                  .filter(col => col.field !== '__check__' && !visibleColumns.includes(col.field))
                  .map((column) => (
                    <Box
                      key={column.field}
                      sx={{
          ...
          <Box sx={{ p: 2 }}>
            ...
            {selectedFilterField?.type === 'multiCheckbox' ? (
              ...
              <TextField
                ...
                variant="standard"
                placeholder={
                  selectedFilterField?.field === 'emailAddress' ? 'Enter email prefix...' :
                  selectedFilterField?.field === 'phoneNumber' ? 'Enter phone number...' :
                  selectedFilterField?.field === 'firstName' || selectedFilterField?.field === 'lastName' ? 'Enter name...' :
                  'Enter value...'
                }
                value={filterValue}
